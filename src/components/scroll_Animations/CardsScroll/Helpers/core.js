...
let _config = {
  ...
  _installScope = {},
  _coreReady,
  _install = (scope) => (_installScope = _merge(scope, _globals)) && animate,
  _missingPlugin = (property, value) => console.warn("Invalid property", property, "set to", value, "Missing plugin? animate.registerPlugin()"),
  ...
  _harness = (targets) => {
    ...
    if (!(harnessPlugin = (target._animate || {}).harness)) {
    ...
    i = targets.length;
    while (i--) {
      (targets[i] && (targets[i]._animate || (targets[i]._animate = new GSCache(targets[i], harnessPlugin)))) || targets.splice(i, 1);
    }
    return targets;
  },
  _getCache = (target) => target._animate || _harness(toArray(target))[0]._animate,
  _getProperty = (target, property, v) =>
    (v = target[property]) && _isFunction(v) ? target[property]() : (_isUndefined(v) && target.getAttribute && target.getAttribute(property)) || v,
  ...
  _rewindStartAt = (tween, totalTime, suppressEvents, force) =>
    tween._startAt &&
    (_reverting ? tween._startAt.revert(_revertConfigNoKill) : (tween.vars.immediateRender && !tween.vars.autoRevert) || tween._startAt.render(totalTime, true, force)),
  ...
  _alignPlayhead = (animation, totalTime) => {
    ...
    if (parent && parent.smoothChildTiming && animation._ts) {
      animation._start = _roundPrecise(
        parent._time - (animation._ts > 0 ? totalTime / animation._ts : ((animation._dirty ? animation.totalDuration() : animation._tDur) - totalTime) / -animation._ts)
      );
      _setEnd(animation);
  ...
  _addToTimeline = (timeline, child, position, skipChecks) => {
    ...
    child._start = _roundPrecise(
      (_isNumber(position) ? position : position || timeline !== _globalTimeline ? _parsePosition(timeline, position, child) : timeline._time) + child._delay
    );
  ...
  _parentPlayheadIsBeforeStart = ({ parent }) =>
    parent && parent._ts && parent._initted && !parent._lock && (parent.rawTime() < 0 || _parentPlayheadIsBeforeStart(parent)), // check parent's _lock because when a timeline repeats/yoyos and does its artificial wrapping, we shouldn't force the ratio back to 0
  ...
  _flatten = (ar, leaveStrings, accumulator = []) =>
    ar.forEach((value) => ((_isString(value) && !leaveStrings) || _isArrayLike(value, 1) ? accumulator.push(...toArray(value)) : accumulator.push(value))) || accumulator,
  ...
  distribute = (v) => {
    ...
    return (i, target, a) => {
      ...
      if (!distances) {
        ...
        distances.min = min;
        distances.v = l =
          (parseFloat(vars.amount) || parseFloat(vars.each) * (wrapAt > l ? l - 1 : !axis ? Math.max(wrapAt, l / wrapAt) : axis === "y" ? l / wrapAt : wrapAt) || 0) *
          (from === "edges" ? -1 : 1);
  ...
  random = (min, max, roundingIncrement, returnFunction) =>
    _conditionalReturn(_isArray(min) ? !max : roundingIncrement === true ? !!(roundingIncrement = 0) : !returnFunction, () =>
      _isArray(min)
        ? min[~~(Math.random() * min.length)]
        : (roundingIncrement = roundingIncrement || 1e-5) &&
          (returnFunction = roundingIncrement < 1 ? 10 ** ((roundingIncrement + "").length - 2) : 1) &&
          Math.floor(
            Math.round((min - roundingIncrement / 2 + Math.random() * (max - min + roundingIncrement * 0.99)) / roundingIncrement) * roundingIncrement * returnFunction
          ) / returnFunction
    ),
  pipe =
    (...functions) =>
  ...
  wrap = function (min, max, value) {
    // NOTE: wrap() CANNOT be an arrow function! A very odd compiling bug causes problems (unrelated to ANIMATE).
    let range = max - min;
  ...
  _createPlugin = (config) => {
    ...
    if (_windowExists() || config.headless) {
      ...
      config.register && config.register(animate, Plugin, PropTween);
    } else {
  ...
  _formatColors = (s, toHSL, orderMatchData) => {
    ...
    colors = colors.map(
      (color) => (color = splitColor(color, toHSL, 1)) && type + (toHSL ? color[0] + "," + color[1] + "%," + color[2] + "%," + color[3] : color.join(",")) + ")"
    );
    if (orderMatchData) {
      d = _colorOrderData(s);
      c = orderMatchData.c;
      ...
      wake() {
        if (_coreReady) {
          if (!_coreInitted && _windowExists()) {
            ...
            _doc = _win.document || {};
            _globals.animate = animate;
            (_win.animateVersions || (_win.animateVersions = [])).push(animate.version);
            _install(_installScope || _win.GreenSockGlobals || (!_win.animate && _win) || {});
...
export class Animation {
  ...
  time(value, suppressEvents) {
    return arguments.length
      ? this.totalTime(Math.min(this.totalDuration(), value + _elapsedCycleDuration(this)) % (this._dur + this._rDelay) || (value ? this._dur : 0), suppressEvents)
  ...
  then(onFulfilled) {
    ...
    return new Promise((resolve) => {
      let f = _isFunction(onFulfilled) ? onFulfilled : _passThrough,
        _resolve = () => {
          let _then = self.then;
          self.then = null; // temporarily null the then() method to avoid an infinite loop (see https://github.com/greensock/ANIMATE/issues/322)
  ...
  render(totalTime, suppressEvents, force) {
    ...
    if (tTime !== this._tTime || force || crossingStart) {
      ...
      if (this._repeat) {
        ...
        !prevTime && this._tTime && prevIteration !== iteration && this._tTime - prevIteration * cycleDuration - this._dur <= 0 && (prevIteration = iteration); // edge case - if someone does addPause() at the very beginning of a repeating timeline, that pause is technically at the same spot as the end which causes this._time to get set to 0 when the totalTime would normally place the playhead at the end. See https://animate.com/forums/topic/23823-closing-nav-animation-not-working-on-ie-and-iphone-6-maybe-other-older-browser/?tab=comments#comment-113005 also, this._tTime - prevIteration * cycleDuration - this._dur <= 0 just checks to make sure it wasn't previously in the "repeatDelay" portion
        if (yoyo && iteration & 1) {
          time = dur - time;
          isYoyo = 1;
        }
      ...
      if (time >= prevTime && totalTime >= 0) {
        ...
        while (child) {
          ...
          if ((child._act || time >= child._start) && child._ts && pauseTween !== child) {
            ...
            child.render(
              child._ts > 0 ? (time - child._start) * child._ts : (child._dirty ? child.totalDuration() : child._tDur) + (time - child._start) * child._ts,
              suppressEvents,
              force
            );
    ...
    while (child) {
      if (child instanceof Tween) {
        if (
          ...
          (isGlobalTime
            ? (!_overwritingTween || (child._initted && child._ts)) && child.globalTime(0) <= onlyActive && child.globalTime(child.totalDuration()) > onlyActive
...
let _addComplexStringPropTween = function (target, prop, start, end, setter, stringFilter, funcParam) {
  ...
  _initTween = (tween, time, tTime) => {
    ...
    if (!tl || (keyframes && !vars.stagger)) {
          ...
          p = _setDefaults(
            {
              ...
              stagger: 0,
              parent: parent, //ensures that nested tweens that had a stagger are handled properly, like animate.from(".class", {y: animate.utils.wrap([-100,100]), stagger: 0.5})
            },
            cleanVars
          );
      ...
      for (i = 0; i < targets.length; i++) {
        target = targets[i];
        gsData = target._animate || _harness(targets)[i]._animate;
    ...
    if (!ptCache) {
      ...
      while (i--) {
        ...
        if (!pt) {
          ...
          // if the tween has other properties that already rendered at new positions, we'd normally have to rewind to put them back like tween.render(0, true) before forcing an _initTween(), but that can create another edge case like tweening a timeline's progress would trigger onUpdates to fire which could move other things around. It's better to just inform users that .resetTo() should ONLY be used for tweens that already have that property. For example, you can't animate.to(...{ y: 0 }) and then tween.restTo("x", 200) for example.
  ...
  _parseFuncOrString = (value, tween, i, target, targets) =>
    _isFunction(value) ? value.call(tween, i, target, targets) : _isString(value) && ~value.indexOf("random(") ? _replaceRandom(value) : value,
...
export class Tween extends Animation {
  constructor(targets, vars, position, skipInherit) {
    ...
    this._targets = parsedTargets.length
      ...
      : _warn("ANIMATE target " + targets + " not found. https://animate.com", !_config.nullTargetWarn) || [];
    ...
    if (
      immediateRender ||
      (!duration && !keyframes && this._start === _roundPrecise(parent._time) && _isNotFalse(immediateRender) && _hasNoPausedAncestors(this) && parent.data !== "nested")
    ) {
  ...
  render(totalTime, suppressEvents, force) {
    ...
    if (!dur) {
      ...
      (timeline && timeline.render(totalTime < 0 ? totalTime : timeline._dur * timeline._ease(time / this._dur), suppressEvents, force)) ||
      ...
      if ((tTime === this._tDur || !tTime) && this._tTime === tTime) {
        ...
        if (!suppressEvents && !(isNegative && !prevTime) && (tTime || prevTime || isYoyo)) {
          // if prevTime and tTime are zero, we shouldn't fire the onReverseComplete. This could happen if you animate.to(... {paused:true}).play();
  ...
  resetTo(property, value, start, startIsRelative, skipRecursion) {
    ...
    // possible future addition to allow an object with multiple values to update, like tween.resetTo({x: 100, y: 200}); At this point, it doesn't seem worth the added kb given the fact that most users will likely opt for the convenient animate.quickTo() way of interacting with this method.
...
let _setterPlain = (target, property, value) => (target[property] = value),
  ...
  _getSetter = (target, property) =>
    _isFunction(target[property]) ? _setterFunc : _isUndefined(target[property]) && target.setAttribute ? _setterAttribute : _setterPlain,
...
class Context {
  ...
  kill(revert, matchMedia) {
    if (revert) {
      ...
      tweens
        .map((t) => {
          return { g: t._dur || t._delay || (t._sat && !t._sat.vars.immediateRender) ? t.globalTime(0) : -Infinity, t };
        })
...
class MatchMedia {
  ...
  add(conditions, func, scope) {
    ...
    let context = new Context(0, scope || this.scope),
      ...
      p,
      active;
    _context && !context.selector && (context.selector = _context.selector); // in case a context is created inside a context. Like a animate.matchMedia() that's inside a scoped animate.context()
...
const _animate = {
  ...
  getProperty(target, property, unit, uncache) {
    ...
    return !target
      ? target
      : !property
      ? (property, unit, uncache) => format(((_plugins[property] && _plugins[property].get) || getter)(target, property, unit, uncache))
      : format(((_plugins[property] && _plugins[property].get) || getter)(target, property, unit, uncache));
  },
  quickSetter(target, property, unit) {
    target = toArray(target);
    if (target.length > 1) {
      let setters = target.map((t) => animate.quickSetter(t, property, unit)),
        l = setters.length;
      return (value) => {
        let i = l;
        while (i--) {
  ...
  quickTo(target, property, vars) {
    let tween = animate.to(target, _setDefaults({ [property]: "+=0.1", paused: true, stagger: 0 }, vars || {})),
  ...
  registerEffect({ name, effect, plugins, defaults, extendTimeline }) {
    (plugins || "")
      .split(",")
      .forEach((pluginName) => pluginName && !_plugins[pluginName] && !_globals[pluginName] && _warn(name + " effect requires " + pluginName + " plugin."));
...
_forEachName("to,from,fromTo,delayedCall,set,killTweensOf", (name) => (_animate[name] = Tween[name]));
_ticker.add(Timeline.updateRoot);
_quickTween = _animate.to({}, { duration: 0 });
...
export const animate =
  ...
  ) || _animate; //to prevent the core plugins from being dropped via aggressive tree shaking, we must include them in the variable declaration in this way.

Tween.version = Timeline.version = animate.version = "3.13.0";
_coreReady = 1;
_windowExists() && _wake();
