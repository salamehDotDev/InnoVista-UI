...
let _win,
  ...
  _setterTransform = (target, property, value) => (target._animate[property] = value),
  _setterScale = (target, property, value) => (target._animate.scaleX = target._animate.scaleY = value),
  _setterScaleWithRender = (target, property, value, data, ratio) => {
    let cache = target._animate;
    ...
    for (i = 0; i < props.length; i += 3) {
      ...
      if (!props[i + 1]) {
        props[i + 2]
          ...
          : style.removeProperty(props[i].substr(0, 2) === "--" ? props[i] : props[i].replace(_capsExp, "-$1").toLowerCase());
      } else if (props[i + 1] === 2) {
  ...
  _getStyleSaver = (target, properties) => {
    ...
    target._animate || animate.core.getCache(target); // just make sure there's a _animate cache defined because we read from it in _saveStyle() and it's more efficient to just check it here once.
  ...
  _createElement = (type, ns) => {
    ...
    return e && e.style ? e : _doc.createElement(type); //some environments won't allow access to the element's style when created with a namespace in which case we default to the standard createElement() to work around the issue. Also note that when ANIMATE is embedded directly inside an SVG file, createElement() won't allow access to the style object in Firefox (see https://animate.com/forums/topic/20215-problem-using-tweenmax-in-standalone-self-containing-svg-file-err-cannot-set-property-csstext-of-undefined/).
  },
  ...
  _initCore = () => {
    if (_windowExists() && window.document) {
      ...
      _reverting = animate.core.reverting;
      _pluginInitted = 1;
    }
  },
  ...
  _getBBox = (target) => {
    ...
    return bounds && !bounds.width && !bounds.x && !bounds.y
      ? { x: +_getAttributeFallbacks(target, ["x", "cx", "x1"]) || 0, y: +_getAttributeFallbacks(target, ["y", "cy", "y1"]) || 0, width: 0, height: 0 }
      : bounds;
  },
  ...
  _tweenComplexCSSString = function (target, prop, start, end) {
    ...
    if (!start || start === "none") {
      // some browsers like Safari actually PREFER the prefixed property and mis-report the unprefixed value like clipPath (BUG). In other words, even though clipPath exists in the style ("clipPath" in target.style) and it's set in the CSS properly (along with -webkit-clip-path), Safari reports clipPath as "none" whereas WebkitClipPath reports accurately like "ellipse(100% 0% at 50% 0%)", so in this case we must SWITCH to using the prefixed property instead. See https://animate.com/forums/topic/18310-clippath-doesnt-work-on-ios/
      ...
      if (s && s !== start) {
        ...
        start = _getComputedProperty(target, "borderTopColor"); // Firefox bug: always reports "borderColor" as "", so we must fall back to borderTopColor. See https://animate.com/forums/topic/24583-how-to-return-colors-that-i-had-after-reverse/
      }
    }
  ...
  _specialProps = {
    ...
    /* className feature (about 0.4kb gzipped).
		, className(plugin, target, property, endValue, tween) {
			...
			if (style.cssText !== cssText) { //only apply if things change. Otherwise, in cases like a background-image that's pulled dynamically, it could cause a refresh. See https://animate.com/forums/topic/20368-possible-animate-bug-switching-classnames-in-chrome/.
				style.cssText = cssText; //we recorded cssText before we swapped classes and ran _getAllStyles() because in cases when a className tween is overwritten, we remove all the related tweening properties from that class change (otherwise class-specific stuff can't override properties we've directly set on the target's style object due to specificity).
			}
			_parseTransform(target, true); //to clear the caching of transforms
			data.css = new animate.plugins.css();
  ...
  _getMatrix = (target, force2D) => {
    let cache = target._animate || _getCache(target),
      style = target.style,
    ...
    if (cache.svg && target.getAttribute("transform")) {
      ...
      if (!parent || (!target.offsetParent && !target.getBoundingClientRect().width)) {
        // note: in 3.3.0 we switched target.offsetParent to _doc.body.contains(target) to avoid [sometimes unnecessary] MutationObserver calls but that wasn't adequate because there are edge cases where nested position: fixed elements need to get reparented to accurately sense transforms. See https://github.com/greensock/ANIMATE/issues/388 and https://github.com/greensock/ANIMATE/issues/375. Note: position: fixed elements report a null offsetParent but they could also be invisible because they're in an ancestor with display: none, so we check getBoundingClientRect(). We only want to alter the DOM if we absolutely have to because it can cause iframe content to reload, like a Vimeo video.
        addedToDOM = 1; //flag
  ...
  _parseTransform = (target, uncache) => {
    let cache = target._animate || new GSCache(target);
    ...
    if (cs.translate) {
      ...
      if (cs.translate !== "none" || cs.scale !== "none" || cs.rotate !== "none") {
        style[_transformProp] =
          (cs.translate !== "none" ? "translate3d(" + (cs.translate + " 0 0").split(" ").slice(0, 3).join(", ") + ") " : "") +
    ...
    cache.x =
      x -
      ((cache.xPercent = x && ((!uncache && cache.xPercent) || (Math.round(target.offsetWidth / 2) === Math.round(-x) ? -50 : 0)))
        ? (target.offsetWidth * cache.xPercent) / 100
        : 0) +
      px;
    cache.y =
      y -
      ((cache.yPercent = y && ((!uncache && cache.yPercent) || (Math.round(target.offsetHeight / 2) === Math.round(-y) ? -50 : 0)))
  ...
  _addRawTransformPTs = (plugin, transforms, target) => {
    ...
    let startCache = _assign({}, target._animate),
...
export const CSSPlugin = {
      ...
      if (specialProp) {
        ...
        if (isTransformRelated) {
          ...
          if (!transformPropTween) {
            cache = target._animate;
            (cache.renderTransform && !vars.parseTransform) || _parseTransform(target, vars.parseTransform); // if, for example, animate.set(... {transform:"translateX(50vw)"}), the _get() call doesn't parse the transform, thus cache.renderTransform won't be set yet so force the parsing of the transform here.
  ...
  getSetter(target, property, plugin) {
    ...
    return property in _transformProps && property !== _transformOriginProp && (target._animate.x || _get(target, "x"))
...
animate.utils.checkPrefix = _checkPropPrefix;
animate.core.getStyleSaver = _getStyleSaver;
...
animate.registerPlugin(CSSPlugin);
