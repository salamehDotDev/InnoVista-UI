...
let animate,
  ...
  _eventTypes,
  _context,
  _getGSAP = () => animate || (typeof window !== "undefined" && (animate = window.animate) && animate.registerPlugin && animate),
  _startup = 1,
  _observers = [],
  _scrollers = [],
  _proxies = [],
  ...
  _getTarget = (t, self) =>
    ((self && self._ctx && self._ctx.selector) || animate.utils.toArray)(t)[0] ||
    (typeof t === "string" && animate.config().nullTargetWarn !== false ? console.warn("Element not found:", t) : null),
  ...
  _getScrollFunc = (element, { s, sc }) => {
    ...
    func.target = element;
    prev || (func.smooth = animate.getProperty(element, "scrollBehavior") === "smooth"); // only set it the first time (don't reset every time a scrollFunc is requested because perhaps it happens during a refresh() when it's disabled in ScrollTrigger.
    return func;
  },
  ...
  _getEvent = (e, preventDefault) => {
    preventDefault && !e._animateAllow && e.preventDefault();
  ...
  _setScrollTrigger = () => {
    ScrollTrigger = animate.core.globals().ScrollTrigger;
    ScrollTrigger && ScrollTrigger.core && _integrate();
  },
  _initCore = (core) => {
    animate = core || _getGSAP();
    if (!_coreInitted && animate && typeof document !== "undefined" && document.body) {
      _win = window;
      _doc = document;
      ...
      _context = animate.core.context || function () {};
...
export class Observer {
  ...
  init(vars) {
    _coreInitted || _initCore(animate) || console.warn("Please animate.registerPlugin(Observer)");
    ...
    this.vars = vars;
    ignore && (ignore = animate.utils.toArray(ignore));
    ...
    onStopDelayedCall = self._dc = animate.delayedCall(onStopDelay || 0.25, onStopFunc).pause();
...
_getGSAP() && animate.registerPlugin(Observer);
